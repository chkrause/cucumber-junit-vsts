...
function convertScenarioToTestCase (scenarioJson, featureJson, options) {
    ...
    if(scenarioJson.steps) {
        ...
        .map(function (stepJson) {
            ...
            if(!stepJson.hidden || (stepJson.hidden && options.printHidden)){
                systemOutString += str;
            }
            
            if (result.duration && (!stepJson.hidden || (stepJson.hidden && options.printHidden))){
 ...
 *  - printHidden - if true will print hiden steps aka before and after
 *
 * @method jsonToXml
...
function cucumberToJunitVSTS (options) {
    ...
    if (options.printHidden === undefined){
