...
function convertScenarioToTestCase (scenarioJson, featureJson, options) {
    var testCaseOutput = [{
        _attr: {
            name: scenarioJson.keyword + ': ' + scenarioJson.name,
            time: 0
        }
    }];
    ...
    if(scenarioJson.steps) {
        ...
        .map(function (stepJson) {
            ...
            systemOutString += stepJson.keyword.trim() + ' ' + stepJson.name + ' (' + stepJson.result.status + ' after ' + (stepJson.result.duration / 1000000000) + ' sec)\n'
            if (result.duration) {
...
function convertFeatureToTestSuite(featureJson, options) {
    ...
    var testSuiteOutput = [{
        _attr: {
            name: featureJson.keyword + ': ' + featureJson.name,
            tests: 0,
            failures: 0,
            skipped: 0
        }
